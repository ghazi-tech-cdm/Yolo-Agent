...
# ---------------- WORKSPACE ----------------
    ...
    optimized_budget=int(p["budget"]*.875)
    score=91
    st.markdown(f'<div class="eyebrow"><span class="pulse"></span>PLAN READY · OPTIMIZED BY AEGIS</div><div class="hero-title" style="font-size:46px">{p["city"]} Getaway</div><p class="muted">A complete customer-ready itinerary with transport, hotel, stay duration, activities and budget.</p>',unsafe_allow_html=True)

    a,b,c,d=st.columns(4)
    for col,title,val in zip([a,b,c,d],["OPTIMIZATION SCORE","TOTAL ESTIMATE","STAY","TRAVELERS"],[f"{score} / 100",f"Rs. {optimized_budget:,}",f'{p["days"]} days / {max(1,p["days"]-1)} nights',str(p["people"])]):
        with col:
    ...
    with left:
        st.markdown('<div class="glass"><div class="smallcaps">TRANSPORT</div><h2>✈ Travel plan</h2>',unsafe_allow_html=True)
        st.markdown("""
        <div class="reason">
            <b>Outbound · AEGIS Air AX-214</b>
            <p>Islamabad (ISB) → Karachi (KHI) · 08:20 — 10:15 · Economy · 2 travelers</p>
        </div>
        <div class="reason">
            <b>Return · AEGIS Air AX-219</b>
            <p>Karachi (KHI) → Islamabad (ISB) · 20:10 — 22:05 · Economy · 2 travelers</p>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:10px;margin-top:14px">
            <span>Estimated flights</span><b>Rs. 18,000</b>
        </div>
        """,unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

        st.write("")
        st.markdown('<div class="glass"><div class="smallcaps">ACCOMMODATION</div><h2>⌂ Hotel selected</h2>',unsafe_allow_html=True)
        st.markdown(f"""
        <div class="reason">
            <b>Pearl Continental Karachi · Deluxe City View</b>
            <p>Check-in: Day 1 · 14:00 &nbsp; | &nbsp; Check-out: Day {int(p["days"])} · 12:00</p>
            <p>{max(1,int(p["days"])-1)} night(s) · Breakfast included · Central location · Rating 4.4/5</p>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:10px;margin-top:14px">
            <span>Hotel estimate</span><b>Rs. 16,000</b>
        </div>
        """,unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

    with right:
        st.markdown('<div class="glass"><div class="smallcaps">STAY OVERVIEW</div><h2>Your trip at a glance</h2>',unsafe_allow_html=True)
        st.markdown(f"""
        <div class="metric" style="margin-bottom:10px"><small>CHECK-IN</small><b>DAY 1 · 14:00</b></div>
        <div class="metric" style="margin-bottom:10px"><small>CHECK-OUT</small><b>DAY {int(p["days"])} · 12:00</b></div>
        <div class="metric"><small>NIGHTS</small><b>{max(1,int(p["days"])-1)}</b></div>
        """,unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

        st.write("")
        st.markdown(f'<div class="glass"><div class="smallcaps">BUDGET</div><h2>Rs. {optimized_budget:,}</h2>',unsafe_allow_html=True)
        budget_items=[("Flights",18000),("Hotel",16000),("Transport",4750),("Food",3000),("Activities",2000)]
        total=sum(v for _,v in budget_items)
        for x,v in budget_items:
            pct=v/total*100
            st.markdown(f'<div style="display:flex;justify-content:space-between;font-size:10px"><span>{x}</span><span>Rs. {v:,}</span></div><div class="progress-wrap" style="margin:6px 0 12px"><div class="progress-bar" style="width:{pct}%"></div></div>',unsafe_allow_html=True)
        st.markdown(f'<div style="display:flex;justify-content:space-between;font-size:10px;margin-top:8px"><b>Planned spend</b><b>Rs. {total:,}</b></div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

    st.write("")
    st.markdown('<div class="glass"><div class="smallcaps">DAY-BY-DAY ITINERARY</div><h2>What the customer actually gets</h2>',unsafe_allow_html=True)
    day_items=[
        ("DAY 1","Arrival + city exploration","10:15 arrival · hotel check-in 14:00 · lunch · Mazar-e-Quaid · Clifton sunset · dinner"),
        ("DAY 2","Culture + coastline","Breakfast · Mohatta Palace · lunch · Dolmen Mall / Clifton · Sea View · dinner")
    ]
    if int(p["days"])>2:
        day_items.append(("DAY 3+","Flexible extension","Breakfast · optional activity · free time · airport transfer for return flight"))
    for day,title,detail in day_items:
        st.markdown(f'<div class="reason"><b>{day} · {title}</b><p>{detail}</p></div>',unsafe_allow_html=True)
    st.markdown('</div>',unsafe_allow_html=True)

    st.write("")
    st.markdown('<div class="glass"><div class="smallcaps">AI DECISIONS</div><h2>Why AEGIS chose these options</h2>',unsafe_allow_html=True)
    for title,txt in [
        ("✈ Flight selected","The flight timings maximize usable destination time while leaving a comfortable return window."),
        ("⌂ Hotel selected","Central location + breakfast + price fit the balanced priority and reduces daily transfer time."),
        ("↗ Stay duration","The itinerary uses the available nights efficiently and keeps check-in/check-out aligned with travel."),
        ("◇ Activities selected","Activities are grouped by area to reduce backtracking and keep the daily schedule realistic."),
        ("₨ Budget protected","The estimated spend stays below the user's limit, leaving a safety margin for unexpected costs.")
    ]:
